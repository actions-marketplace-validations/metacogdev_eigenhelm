...
class CompositionManifest:
    """A manifest that references other corpus manifests by path."""
    ...
    def __post_init__(self) -> None:
        for fname in ("name", "version", "created"):
            if not getattr(self, fname):
                raise ValueError(f"composition.{fname} must be non-empty")
        if not self.sources:
            raise ValueError("composition.sources must be non-empty")
        for src in self.sources:
            if not src:
                raise ValueError("composition.sources contains an empty path")
            if not src.endswith(".toml"):
                raise ValueError(
                    f"composition.sources: '{src}' must have .toml extension"
                )

    @classmethod
    def from_dict(cls, d: dict) -> CompositionManifest:
        """Parse a TOML-decoded dict into a validated CompositionManifest."""
        comp = d.get("composition", {})
        for fname in ("name", "version", "created"):
            if fname not in comp:
                raise ValueError(f"composition.{fname} is required")
        if "sources" not in comp:
            raise ValueError("composition.sources is required")
        return cls(
            name=comp["name"],
            version=comp["version"],
            created=comp["created"],
            sources=tuple(comp["sources"]),
        )

    def resolve(self, base_dir: Path) -> list[CorpusManifest]:
        """Resolve source paths relative to base_dir and load each manifest.

        Raises:
            FileNotFoundError: If a source file does not exist.
            ValueError: If a source file is not a valid corpus manifest.
        """
        ...
        for src in self.sources:
            ...
            if not src_path.exists():
                raise FileNotFoundError(
                    f"composition source not found: {src_path}"
                )
            manifests.append(load_manifest(src_path))
        return manifests


def load_composition(path: str | Path) -> CompositionManifest:
    """Parse and validate a TOML composition manifest file.
    ...
    return CompositionManifest.from_dict(data)


def load_any_manifest(path: str | Path) -> CorpusManifest | CompositionManifest:
    """Load a TOML file as either a CorpusManifest or CompositionManifest.

    Dispatches on the presence of ``[composition]`` vs ``[corpus]`` header.

    Raises:
        FileNotFoundError: If ``path`` does not exist.
        ValueError: If the file contains neither header, or is invalid.
        tomllib.TOMLDecodeError: If the file is not valid TOML.
    """
    path = Path(path)
    with open(path, "rb") as fh:
    ...
    raise ValueError(
        f"{path}: expected [corpus] or [composition] header, found neither"
    )
